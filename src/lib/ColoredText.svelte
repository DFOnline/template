<script lang="ts">
	...
	interface component extends style {
		text: string;
	}
...
<span>
	{#each parse(text) as part}
		<span
			class={orEmpty`bold${part.bold}italic${part.italic}underlined${part.underlined}strikethrough${part.strikethrough}obfuscated${part.obfuscated}`}
			style={`${part.color != null ? `color: ${part.color};` : ''}`}>{part.text}</span
		>
	{/each}
</span>
