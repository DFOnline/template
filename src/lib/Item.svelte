...
<div
	class={`item ${item.id}`}
	style={customIcon != null ? `background-image: url(${endPoint}/${customIcon}.png)` : undefined}
	{draggable}
	on:dragstart
	on:dragend
	role="cell"
	...
	<span class="tooltip">
		...
		{#if item.id == 'bl_tag'}
			...
			{#if actiondump != null}
				{#each tag?.options ?? [] as option}
					<br />
					<span class={item.data.option == option.name ? 'aqua' : 'lg'}
						>{#if item.data.option == option.name}<span class="cyan">»</span>
						{/if}{option.name}</span
					>
				{/each}
			{/if}
		...
		{#if item.id == 'pn_el'}
			...
			<span class="dg"
				...
				<span class="lg">{item.data.description ?? item.data.name}</span></span
			> <br />
			{#if item.data.default_value != null}
				<span class="lg"><span class="blue">⏵</span> Default = </span>
				<span
					style="--miniSlotSize: var(--slot-size,calc(var(--tooltip-scale,2) * 1px)); width: var(--miniSlotSize); height: var(--miniSlotSize); display: block;"
				>
					<svelte:self item={item.data.default_value} {actiondump} {endPoint}></svelte:self>
				</span>
			{/if}
		{/if}
		{#if item.id == 'loc'}
		...
		{#if item.id == 'pot'}
			...
			<br /> <span class="lg">Duration: </span>
			<span
				>{item.data.dur >= 1000000
					? 'Infinite'
					: item.data.dur % 20 == 0
						? `${Math.floor(item.data.dur / 20 / 60)}:${String((item.data.dur / 20) % 60).padStart(2, '0')}`
...
<style>
	...
	.txt {
		background-image: url(./media/items/txt.png);
	}
	.comp {
		background-image: url(./media/items/comp.png);
	}
	.num {
		background-image: url(./media/items/num.png);
	}
	.var {
		background-image: url(./media/items/var.png);
	}
	.snd {
		background-image: url(./media/items/snd.png);
	}
	.pot {
		background-image: url(./media/items/pot.png);
	}
	.loc {
		background-image: url(./media/items/loc.png);
	}
	.vec {
		background-image: url(./media/items/vec.png);
	}
	.part {
		background-image: url(./media/items/part.png);
	}
	.g_val {
		background-image: url(./media/items/g_val.png);
	}
	.pn_el {
		background-image: url(./media/items/pn_el.png);
	}
	.hint {
		background-image: url(./media/items/hint.png);
	}
